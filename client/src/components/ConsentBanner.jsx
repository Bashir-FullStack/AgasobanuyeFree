...
export default function ConsentBanner() {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("hiromart
...
_cookie_consent");
  ...
  const accept = () => {
    try { localStorage.setItem("hiromart

_cookie_consent", "accepted"); } catch {}
    setVisible(false);
  };
...
_cookie_consent", "rejected"); } catch {}
    setVisible(false);
  };
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 p-4" style={{ animation: "slideUp 0.3s ease-out" }}>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl mx-auto max-w-lg p-5">
        ...
        <h3 className="text-base font-bold text-gray-900 mb-1">{t("cookie.title")}</h3>
