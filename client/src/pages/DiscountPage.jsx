...
export default function DiscountPage() {
  const { data: products, loading } = useFetch("/products/discounts");
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="bg-gradient-to-r from-primary/10 to-primary/5 border-b border-gray-100">
        <div className="max-w-[1000px] mx-auto px-4 py-10">
          <h1 className="text-2xl font-bold text-dark">Discounts & Deals</h1>
          <p className="text-sm text-gray-500 mt-1">{discounted.length} products on sale</p>
        </div>
      </div>
        ...
        <div className="border-2 border-gray-200 rounded-xl overflow-hidden">
          <div className="-mr-[2px] -mb-[2px]">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
              {discounted.length === 0 ? (
                <div className="col-span-full py-20 text-center text-gray-500 text-sm">No discounts available right now.</div>
              ) : discounted.map((p) => (
