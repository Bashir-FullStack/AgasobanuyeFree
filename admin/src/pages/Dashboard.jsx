...
export default function Dashboard() {
  ...
  const [widgets, setWidgets] = useState(() => {
    try {
      const saved = localStorage.getItem("hiromart_dashboard_widgets");
      return saved ? JSON.parse(saved) : {
        stats: true, recentOrders: true, topProducts: true,
        quickActions: true, recentActivity: true, storeOverview: true, storeHealth: true,
      };
    } catch { return { stats: true, recentOrders: true, topProducts: true, quickActions: true, recentActivity: true, storeOverview: true, storeHealth: true }; }
  });
  ...
  const toggleWidget = (key) => {
    setWidgets((prev) => {
      ...
      localStorage.setItem("hiromart_dashboard_widgets", JSON.stringify(next));
      return next;
    });
  };

  const WidgetTogglePanel = () => (
    <div className="relative">
      <button
        onClick={() => setWidgetOpen(!widgetOpen)}
        className="p-2 rounded-xl transition-all hover:bg-[var(--bg-page)]"
      ...
      {widgetOpen && (
        <>
          <div className="fixed inset-0 z-10" onClick={() => setWidgetOpen(false)} />
          <div className="absolute top-full right-0 mt-2 w-56 rounded-2xl shadow-2xl z-20 overflow-hidden" style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border-color)" }}>
            <div className="p-3 font-bold text-sm" style={{ color: "var(--text-primary)", borderBottom: "1px solid var(--border-color)" }}>Dashboard Widgets</div>
            <div className="p-2 space-y-0.5">
              {[
                ...
                { key: "storeOverview", label: "Store Overview" },
                { key: "storeHealth", label: "Store Health" },
              ].map((w) => (
                <label key={w.key} className="flex items-center justify-between px-3 py-2 rounded-xl cursor-pointer hover:bg-[var(--bg-page)] transition">
                  <span className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>{w.label}</span>
                  <button
                    onClick={() => toggleWidget(w.key)}
                    className={`relative w-9 h-5 rounded-full transition-colors ${widgets[w.key] ? "bg-[#2275fc]" : "bg-gray-300 dark:bg-gray-600"}`}
                  >
                    <span className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${widgets[w.key] ? "translate-x-4" : "translate-x-0"}`} />
                  </button>
                </label>
              ))}
            </div>
