...
router.post('/', async (req, res) => {
  try {
    const { data, error } = await supabase.from('promos').insert(req.body).select();
    if (error) throw error;
...
router.put('/:id', async (req, res) => {
  try {
    const { data, error } = await supabase.from('promos').update(req.body).eq('id', req.params.id).select();
    if (error) throw error;
